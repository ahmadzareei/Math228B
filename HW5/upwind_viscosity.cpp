...
void upwind_viscosity_step(int Nx, double l, double* x, double* u_new, double *u_old,double epsilon, double dx);
...
int main () {
  ...
  while ( t<t_end){
    ...
    upwind_viscosity_step(Nx,l,x,u_new,u_old,epsilon,dx);
...
//*******************************************************************************
void upwind_viscosity_step(int Nx, double l, double* x, double* un, double* u,double epsilon,double dx){
  ...
  for ( int i=1;i<Nx-1;i++){
    ...
    un[i] = u[i] - l*(ap*(u[i+1]-u[i]) + am * (u[i]-u[i-1]))+ epsilon * 0.5* l * (u[i+1]-2.0*u[i] + u[i-1]);
    */
    if(u[i] >0)
      un[i] = u[i] - l*(u[i+1]*u[i+1]-u[i]*u[i])*0.5 +epsilon *  l/dx * (u[i+1]-2.0*u[i] + u[i-1]) ;
    else
      un[i] = u[i] - l*(u[i+1]*u[i+1]-u[i]*u[i])*0.5 +epsilon *  l/dx * (u[i+1]-2.0*u[i] + u[i-1]) ;
  }
