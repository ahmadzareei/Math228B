...
void lax_fredrichs_step(int Nx, double l, double* x, double* u_new, double *u_old);
...
int main () {
  ...
  while ( t<t_end){
    ...
    lax_fredrichs_step(Nx,l,x,u_new,u_old);
...
//*******************************************************************************
void lax_fredrichs_step(int Nx, double l, double* x, double* un, double* uo){
  
  for ( int i=1;i<Nx-1;i++){
    un[i] = 0.5*( uo[i+1] + uo[i-1] ) 
      -l/2.0*0.5*(pow(uo[i+1],2) - pow(uo[i-1],2));
