...
int main(){
  ...
  double temp, temp2;
  out.open("final.dat");
  ofstream out_m, out_m2,out_m3;
  out_m.open("middle.dat");
  out_m2.open("middle2.dat");
  out_m3.open("middle3.dat");
  ofstream out2;
  ...
  curve_x=new double[N+1];
  curve_y=new double[N+1];
  ...
  for(int i=0;i<N_section;i++){
    s=0.0+ds*(double)i;
    curve_x[i] = s; curve_y[i] = cos(M_PI*2.0*s);

    s=s+1.0;
    curve_x[i+N_section] = 1.0; curve_y[i+N_section] = 1.0-3.0*(s-1.0);

    s=s+1.0;
    curve_x[i+2*N_section] = 3.0-s; curve_y[i+2*N_section] = -2.0;

    s=s+1.0;
    curve_x[i+3*N_section] = 0.0; curve_y[i+3*N_section] = -2.0+3.0*(s-3.0);    
  }
  /*
  for(int i=0;i<N;i++){
  ...
  */
  ...
  for(int step = 0; step<Steps;step++){
	...
	temp2 = temp2/sqrt(pow(px,2)+ pow(py,2)+1e-6);
	temp2 = (A - epsilon * temp2);
	...
	temp = fmax(temp2,0.0)*delp + fmin(temp2,0.0)*delm;
	 ...
	 */
	 phin[I(i,j)] = phi[I(i,j)] -dt*temp;// + epsilon *dt *temp2;;
