...
void Leap_Frog_Step(double*w1, double* w2, double* w1_new, double* w2_new,double dt,double dx,double *a,int N);
...
int main(){
  ...
  for(int i=1;i<N-1;i++){
    w1[i] = sqrt(a[i]+1.)/2.0*(u[i]-u[i-1])/(dx); //sqrt(a[i]+1.)/2.0*(u[i+1]-u[i-1])/(2.0*dx);
    w2[i]= -sqrt(a[i]+1.)/2.0*(u[i]-u[i-1])/(dx);//-sqrt(a[i]+1.)/2.0*(u[i+1]-u[i-1])/(2.0*dx);
  }
  ...
  while(t<T){
    Leap_Frog_Step(w1,w2,w1_new,w2_new,dt,dx,a,N);
...
void Leap_Frog_Step(double*w1, double* w2, double* w1_new, double* w2_new,double dt,double dx,double *a,int N){
  ...
  for(int i=1;i<N-1;i++){
    w1[i] = w1[i] + 2*0.5*dt/dx*sqrt(a[i])*(w1_new[i+1]-w1_new[i-1]);
    w2[i] = w2[i] - 2*0.5*dt/dx*sqrt(a[i])*(w2_new[i+1]-w2_new[i-1]);
  }
  // w2 is going right and w1 is going left
  // we solve w1 at first point and w2 at end point 
  // Why is it not working :)
  w1[0] = w1_new[0] + dt/dx*sqrt(a[0])*(w1_new[1]-w1_new[0]);
  w2[N-1] = w2_new[N-1] - dt/dx*sqrt(a[N-1])*(w2_new[N-1]-w2_new[N-2]);
  ...
  for(int i=0;i<N;i++){
    temp = w1_new[i]; w1_new[i] = w1[i]; w1[i] = temp;
    temp = w2_new[i]; w2_new[i] = w2[i]; w2[i] = temp;
  }  
}
/******************************************************************************************/
double Energy_Compute(double *w1,double *w2,double *u,double *a, double dt,double dx,int N){
  ...
  Integrator(w1,w2,u,a,N);
  for(int i=0;i<N;i++){
    energy = energy +  0.5*a[i]/(a[i]+1.)*(w1[i]+w2[i])*(w1[i]+w2[i])*dx + 0.5*1.0/(a[i]+1.0)*(w1[i]-w2[i])*(w1[i]-w2[i])*dx; //0.5*((u[i]) * (u[i]))*dx + 0.5*a[i]/(a[i]+1)*((w1[i]-w2[i])*(w1[i]-w2[i]))*dx ;
  }
  return energy;
}
...
void Lax_Wandroff_Step(double*w1, double* w2, double* w1_new, double* w2_new,double dt,double dx,double *a,int N){
  ...
  w2_new[N-1] = w2[N-1] - dt/dx*sqrt(a[N-1])*(w2[N-1]-w2[N-1]);// + 0.5*a[N-1]*dt*dt/dx/dx*(w2[N-1] -2.0*w2[N-2]+ w2[N-3]);
