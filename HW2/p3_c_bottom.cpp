...
void step_diffusion_solver(int N, double*** u,double*** u_new, double landa,double tt,double f);
...
int main(){
  ...
  int kk =1;
    ...
    while(t<100.0){
      tt = t - floor(t);
      step_diffusion_solver(N,u,u_new,landa,tt,fraction);
      average= mean_value(N,u);
      myfile<<t<<"\t"<<12.0+average<<"\n";
      t=t+k;
    }
...
double  mean_value(int N, double*** u){
  ...
  for(int i=1;i<N-1;i++)
    for(int l=1;l<N-1;l++)
      //      for(int m=1;m<N-1;m++)
	ave =ave + u[i][l][1];
  ave = ave/((double)N-2.0)/((double)N-2.0);
  return ave;
}
